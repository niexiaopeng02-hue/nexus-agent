...
async def seed_business_data(session: AsyncSession) -> None:
    ...
    for item in [*orders, *products]:
        if not await session.get(type(item), item.id):
            session.add(item)
    for item in inventory:
        if not await session.get(tables.Inventory, item.product_id):
            session.add(item)
...
async def seed_knowledge_base(session: AsyncSession, provider: LLMProvider) -> None:
    ...
    for name, text in load_sample_texts():
        existing = await session.scalar(select(tables.Document.id).where(tables.Document.name == name).limit(1))
        if not existing:
